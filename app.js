...
function renderProducts() {
    ...
    STATE.products.forEach(p => {
        ...
        card.innerHTML = `
            <div>
                <h3 class="font-bold text-gray-900 leading-tight mb-1 line-clamp-2">${p.name}</h3>
                <p class="text-xs text-gray-400">${p.category || 'Khác'}</p>
            </div>
            <div class="flex items-end justify-between">
                <div>
                    <p class="text-blue-600 font-bold text-lg">${formatCurrency(p.price)}</p>
                    <p class="text-xs text-gray-400">Còn: ${p.stock}</p>
                </div>
                <button class="add-to-cart-btn w-8 h-8 rounded-lg bg-blue-600 text-white flex items-center justify-center hover:bg-blue-700 active:scale-90 transition-transform">
...
function switchInventoryTab(tab) {
    ...
    if (tab === 'products') {
        const prodBtn = document.querySelector('[onclick="switchInventoryTab(\'products\')"]');
        if (prodBtn) {
            prodBtn.classList.add('active', 'border-blue-600', 'text-blue-600');
            prodBtn.classList.remove('border-transparent', 'text-gray-500');
        }
        document.getElementById('inventory-products-tab').classList.remove('hidden');
        document.getElementById('inventory-products-tab').classList.add('flex');
        document.getElementById('inventory-categories-tab').classList.add('hidden');
        document.getElementById('inventory-categories-tab').classList.remove('flex');
    } else {
        const catBtn = document.querySelector('[onclick="switchInventoryTab(\'categories\')"]');
        if (catBtn) {
            catBtn.classList.add('active', 'border-blue-600', 'text-blue-600');
            catBtn.classList.remove('border-transparent', 'text-gray-500');
        }
        document.getElementById('inventory-products-tab').classList.add('hidden');
        document.getElementById('inventory-products-tab').classList.remove('flex');
        document.getElementById('inventory-categories-tab').classList.remove('hidden');
        document.getElementById('inventory-categories-tab').classList.add('flex');
        renderCategoryList();
    }
}
...
function renderCategoryList() {
    ...
    const displayCategories = STATE.categories.filter(c => c !== "Tất cả");

    if (displayCategories.length === 0) {
        container.innerHTML = '<div class="bg-white p-8 rounded-xl text-center text-gray-400"><i class="ph ph-folder-open text-4xl mb-2"></i><p>Chưa có nhóm hàng. Bấm "Thêm nhóm"!</p></div>';
        return;
    }
        ...
        card.innerHTML = `
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center">
                    <i class="ph ph-tag text-green-600 text-xl"></i>
                </div>
                <div>
                    <h4 class="font-bold text-gray-800">${cat}</h4>
                    <p class="text-xs text-gray-400">Nhóm hàng hóa</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <button type="button" class="edit-cat-btn p-2 hover:bg-gray-100 rounded-lg text-gray-600" data-name="${cat}">
                    <i class="ph ph-pencil-simple text-lg"></i>
                </button>
                <button type="button" class="delete-cat-btn p-2 hover:bg-red-50 rounded-lg text-gray-600" data-name="${cat}">
    ...
    container.querySelectorAll('.delete-cat-btn').forEach(btn => {
        btn.addEventListener('click', () => confirmDeleteCategory(btn.dataset.name));
    });
}

function openCategoryModal() {
    if (!STATE.isAdmin) {
        showLoginModal(() => openCategoryModal());
        return;
    }
    ...
    document.getElementById('category-modal-title').innerText = "Thêm nhóm hàng mới";
...
function saveCategory() {
    ...
    if (id) {
        ...
        // Add new - insert before "Khác"
        const khacIndex = STATE.categories.indexOf("Khác");
        if (khacIndex !== -1) {
            STATE.categories.splice(khacIndex, 0, name);
        } else {
    ...
    renderCategoryList();
    renderCategories();
    showToast(`Đã lưu nhóm "${name}"`, "success");
}

function editCategory(categoryName) {
    if (!STATE.isAdmin) {
        showLoginModal(() => editCategory(categoryName));
        return;
    }
    const modal = document.getElementById('category-modal');
    if (!modal) return;
    ...
    document.getElementById('category-modal-title').innerText = "Sửa nhóm hàng";
...
function confirmDeleteCategory(categoryName) {
    const productCount = STATE.products.filter(p => p.category === categoryName).length;
    let message = `Bạn chắc chắn muốn xóa nhóm "${categoryName}"?`;
    if (productCount > 0) {
        message += `\n\nCó ${productCount} sản phẩm đang thuộc nhóm này.\nCác sản phẩm sẽ được chuyển sang nhóm "Khác".`;
    }
    ...
    renderCategoryList();
    renderCategories();
    showToast(`Đã xóa nhóm "${categoryName}"`, "success");
}
